    ...
    <div class="h-screen">
        ...
        <div class="my-5 mx-24 px-5">
            ...
            <div class="full my-4 bg-gray-200">
                <p class="text-center p-2 font-semibold text-lg" v-if="!isUnpaid"> ADD PAYMENT </p>
                <p class="text-center p-2 font-semibold text-lg" v-else> ADD ENTRY for UNPAID MONTHS </p>
            </div>

            <!-- FOR REGULAR PAYMENT -->
            <div v-if="!isAddingPayment && !isUnpaid">
                ...
                <div class="full my-2 flex">
                    <div class="w-1/2 mr-2"> <text-area label="Remarks / Description" v-model="new_payment.remarks" /> </div>
                    <div class="w-1/2 ml-2">
                        <label-component label="NOTES (e.g. payment breakdowns, etc." class="mb-1" />
                        <textarea class="p-2 w-full border rounded-md" v-model="new_payment.notes" rows=5 > </textarea>
                    </div>
                </div>
                <div class="my-2">
                    <label class="inline-flex items-center">
                        <!-- <input type="checkbox" class="form-checkbox" v-model="new_payment.isDeductible" :disabled="new_payment.or_ar_no.length <= 0"/> -->
            ...
            <!-- ADD UNPAID ENTRY FORM -->
            <div v-else-if="!isAddingPayment && isUnpaid">
                <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 py-1">
                    <div class="flex gap-4">
                        <div class="w-2/5"> <readonly-form label="Last Name" :value="buyer.last_name" /> </div>
                        <div class="w-2/5"> <readonly-form label="First Name" :value="buyer.first_name" /> </div>
                        ...
                        <div class="grid grid-cols-3 mb-4">
                            <div class="mx-auto w-3/4">
                                ...
                                <div class="mx-auto w-fullmt-1 relative rounded-md shadow-sm border-gray-200">
                                    <input type="text"
                                    :value="formatDisplay(new_payment.consecutive_unpaid_months)"
                    ...
                    <div class="grid grid-cols-1">
                        ...
                        <div class="full flex gap-4">
                            ...
                            <div class="w-1/2">
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm border-gray-200">
                                    <input type="text"
                                        :value="formatDisplay(new_payment.total_unpaid)"
                                        class="w-full py-2 px-4 text-md border border-gray-200 rounded-md uppercase"
                                        readonly disabled>
                                </div>
                            </div>
                ...
                <div class="flex items-center mx-auto justify-center gap-8 my-8">
                    <button type="button" v-on:click="submitPayment"
                        class="bg-gray-500 p-4 w-1/4 align-middle text-white font-bold border rounded-md mb-4">
                        SUBMIT PAYMENT
                    </button>
            ...
            <div v-else-if="isAddingPayment" class="text-center mx-auto my-4 font-semibold text-lg">
                ADDING PAYMENT...
            </div>
        </div>
    </div>
...
<script>
    ...
    export default ({
        ...
        data() {
            return {
                ...
                new_payment: {
                    ...
                    remarks: '',
                    notes: '',
                    // isDeductible: this.$route.params.isUnpaid ? 0 : 1,
                    isDeductible: 0,
        ...
        methods: {
            ...
            calculateUnpaid() {
                if(this.isUnpaid) {
                    // this.new_payment.penalty = this.formatParsedFloat(parseFloat(this.new_payment.amount) * parseFloat(0.02) * parseFloat(this.new_payment.consecutive_unpaid_months))
                    this.new_payment.total_unpaid = this.formatParsedFloat(parseFloat(this.new_payment.amount) + parseFloat(this.new_payment.penalty))
                    this.updateCalculatedPenalty()
                }
            },
            formatDisplay(value) {
               return value ? value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : value        
                // console.log('FORMAT DISP', value)       
            //    return value ? `${(value.toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}` : value
            },
            formatDisplayWithoutComma(value) {
               return value ? `${(value.toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}` : value
            },
            ...
            updateCalculatedPenalty() {
                this.calculatedPenalty =  this.formatParsedFloat(parseFloat(this.new_payment.amount) * parseFloat(0.02) * parseFloat(this.new_payment.consecutive_unpaid_months))
            },
            updatePenalty(event) {
            ...
            updateUnpaidMonths(event) {
                this.new_payment.consecutive_unpaid_months = event.target.value
                this.calculateUnpaid()
            },
            submitPayment() {
                if(!this.new_payment.payment_date) {
                    ...
                    this.new_payment.total_unpaid = this.formatParsedFloat(this.new_payment.total_unpaid)
